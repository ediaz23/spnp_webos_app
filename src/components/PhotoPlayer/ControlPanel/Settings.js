...
const Settings = (props) => {
    ...
    const handleClick = e => {
        let newState = null
        const value = options[header][e.selected]
        setIndex(e.selected);
        if (header.toUpperCase() === 'SIZE') {
            newState = { settings, currentSettings: { ...currentSettings, Size: value } }
        } else if (header.toUpperCase() === 'TRANSITION') {
            newState = { settings, currentSettings: { ...currentSettings, Transition: value } }
        } else if (header.toUpperCase() === 'SPEED') {
            newState = { settings, currentSettings: { ...currentSettings, Speed: value } }
        }
        if (newState) {
